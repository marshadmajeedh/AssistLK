...
| **Agent Responsibility** | `ProblemUnderstandingAgent` (.NET 8 native). Uses `GeminiService` (Google Gemini 2.5 Flash) with tool augmentation (`LocationExtractionTool`, `ProblemClassificationTool`, `ServiceKnowledgeTool`). Produces structured `ProblemUnderstandingOutput` (category, summary, urgency, confidence, follow-up questions). |
