...
class MockServiceRequestService extends ServiceRequestService {
  ...
  Future<ProblemUnderstandingResultModel> analyze(String id) async {
    ...
    final index = mockRequests.indexWhere(
      (request) => request.serviceRequestId == id,
    );
    if (index >= 0) {
...
void main() {
  ...
  Widget buildApp(Widget child) {
    return MultiProvider(
      providers: [
        ChangeNotifierProvider<CustomerLocationProvider>(
  ...
  group('Customer Home and Activity', () {
    testWidgets(
      'displays welcome banner and empty state when no requests exist',
      (tester) async {
        await tester.pumpWidget(
          buildApp(const Scaffold(body: CustomerHomeScreen())),
        );
        await tester.pumpAndSettle();
        expect(find.text('Welcome, Kamal Perera'), findsOneWidget);
        expect(
          find.text('kamal@assistlk.com'),
          findsNothing,
        ); // Email belongs to Account.
        await requestProvider.loadMyRequests();
        await tester.pumpWidget(
          buildApp(const Scaffold(body: CustomerActivityScreen())),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets('displays service request list when requests exist', (
      ...
      expect(tester.takeException(), isNull);
      expect(
        find.widgetWithText(ServiceCategoryCard, 'Plumbing'),
        findsNothing,
      ); // Catalog moved to Services.
      expect(
        find.widgetWithText(ServiceRequestCard, 'Plumbing'),
        findsOneWidget,
      );
      expect(
        find.text('Bathroom tap is dripping continuously'),
        findsOneWidget,
      );
    ...
    testWidgets(
      'header Create Request button has finite content-safe constraints and survives mouse hover on desktop viewport',
      (tester) async {
        ...
        final headerButtonFinder = find
            .widgetWithText(ElevatedButton, 'Create Request')
            .first;
        ...
        expect(
          renderBox.constraints.hasBoundedWidth,
          isFalse,
        ); // Parent Row provides unbounded max width
        expect(
          renderBox.constraints.minWidth,
          0.0,
        ); // Never infinite minimum width!
    ...
    testWidgets(
      'renders cleanly on narrow viewport without overflow or layout exceptions',
      (tester) async {
        ...
        expect(
          find.widgetWithText(ElevatedButton, 'Create Request').first,
          findsOneWidget,
        );
      },
    );
    ...
    testWidgets(
      'populated state renders cleanly on desktop viewport with pointer movement',
      (tester) async {
        tester.view.physicalSize = const Size(1280, 800);
        tester.view.devicePixelRatio = 1.0;
        addTearDown(() {
        ...
        await gesture.moveTo(
          tester.getCenter(find.widgetWithText(ServiceRequestCard, 'Plumbing')),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets(
      ...
      (tester) async {
        ...
        await gesture.moveTo(
          tester.getCenter(find.text('Create Request').first),
        );
        await tester.pump();
        await gesture.moveTo(tester.getCenter(find.text('My Requests')));
        await tester.pump();
        await gesture.moveTo(
          tester.getCenter(find.text('Bathroom tap is dripping continuously')),
        );
        await tester.pump();
      },
    );
  ...
  group('CreateServiceRequestScreen 3-Step Guided Flow', () {
    testWidgets('validates required inputs and submits across 3-step flow', (
      tester,
    ) async {
      ...
      expect(
        find.text('Please provide at least 10 characters.'),
        findsOneWidget,
      );
      ...
      expect(
        mockService.mockRequests.first.locationText,
        '123 Galle Road, Colombo 03',
      );
    });

    testWidgets(
      'supports back navigation between steps while preserving entered data',
      (tester) async {
        await tester.pumpWidget(buildApp(const CreateServiceRequestScreen()));
        await tester.pumpAndSettle();
    ...
    testWidgets(
      'renders initial category preference banner and supports changing preference',
      (tester) async {
        await tester.pumpWidget(
          buildApp(
    ...
    testWidgets(
      'renders Let AssistLK AI identify banner when no preference is provided',
      (tester) async {
        await tester.pumpWidget(
          buildApp(
            const CreateServiceRequestScreen(initialCategoryPreference: null),
          ),
        );
        await tester.pumpAndSettle();
  ...
  group('CustomerServicesScreen Service Shortcuts and AI Option', () {
    testWidgets(
      'renders exactly 4 canonical category shortcuts and no prohibited categories',
      (tester) async {
        await tester.pumpWidget(
          buildApp(const Scaffold(body: CustomerServicesScreen())),
        );
        await tester.pumpAndSettle();
        ...
        expect(
          find.widgetWithText(ServiceCategoryCard, 'Electrical'),
          findsOneWidget,
        );
        expect(
          find.widgetWithText(ServiceCategoryCard, 'Vehicle Assistance'),
          findsOneWidget,
        );
        expect(
          find.widgetWithText(ServiceCategoryCard, 'Appliance Repair'),
          findsOneWidget,
        );
    ...
    testWidgets('renders prominent AI problem understanding card', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildApp(const Scaffold(body: CustomerServicesScreen())),
    ...
    testWidgets(
      'tapping Plumbing shortcut navigates with Plumbing preference',
      (tester) async {
        await tester.pumpWidget(
          buildApp(const Scaffold(body: CustomerServicesScreen())),
        );
    ...
    testWidgets(
      'tapping Vehicle Assistance shortcut navigates with canonical Vehicle Repair',
      (tester) async {
        await tester.pumpWidget(
          buildApp(const Scaffold(body: CustomerServicesScreen())),
        );
    ...
    testWidgets(
      'tapping AI option card navigates with Let AssistLK AI identify mode',
      (tester) async {
        await tester.pumpWidget(
          buildApp(const Scaffold(body: CustomerServicesScreen())),
        );
  ...
  group('ServiceRequestDetailScreen Status-Driven Logic', () {
    testWidgets('Created status shows Analyze with AssistLK AI button', (
      tester,
    ) async {
    ...
    testWidgets(
      'ServiceRequestDetailScreen_WhenBackendStatusAnalyzing shows progress indicator and does not show Analyze button',
      (tester) async {
        final sample = ServiceRequestModel(
        ...
        await tester.pumpWidget(
          buildApp(
            const ServiceRequestDetailScreen(requestId: 'req-analyzing'),
          ),
        );
        await tester.pump();
        await tester.pump();
    ...
    testWidgets(
      'ServiceRequestDetailScreen_WhenProviderIsAnalyzing shows progress indicator and does not show Analyze button',
      (tester) async {
        final sample = ServiceRequestModel(
        ...
        mockService.mockRequests = [sample];
        mockService.analyzeCompleter =
            Completer<ProblemUnderstandingResultModel>();

        await tester.pumpWidget(
          buildApp(
            const ServiceRequestDetailScreen(requestId: 'req-c-loading'),
          ),
        );
        await tester.pumpAndSettle();
    ...
    testWidgets(
      'Successful clarification result sets AwaitingInformation and displays clarification state',
      (tester) async {
        final sample = ServiceRequestModel(
        ...
        expect(
          find.text('Is the noise continuous or only when heat turns on?'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'Analyzed status shows AnalysisResultCard and Mark Ready button',
      (tester) async {
        final sample = ServiceRequestModel(
    ...
    testWidgets(
      'analysisStateNeedsRefresh true shows uncertain card, hides Analyze, Edit, Cancel',
      (tester) async {
        final sample = ServiceRequestModel(
        ...
        await tester.pumpWidget(
          buildApp(
            const ServiceRequestDetailScreen(requestId: 'req-uncertain'),
          ),
        );
        await tester.pumpAndSettle();
        ...
        expect(
          find.text('Unable to confirm the latest analysis status.'),
          findsOneWidget,
        );
    ...
    testWidgets(
      'manual Refresh Status restores Analyze button if backend returned Created',
      (tester) async {
        final sample = ServiceRequestModel(
    ...
    testWidgets(
      'manual Refresh Status shows progress state if backend returned Analyzing',
      (tester) async {
        final sample = ServiceRequestModel(
    ...
    testWidgets(
      'manual Refresh Status shows clarification state if backend returned AwaitingInformation',
      (tester) async {
        final sample = ServiceRequestModel(
    ...
    testWidgets(
      'timeout followed by reconciliation reaching Analyzed shows AnalysisResultCard and mismatch banner',
      (tester) async {
        final sample = ServiceRequestModel(
          ...
          description:
              'Water is leaking heavily from the pipe under my kitchen sink.',
          locationText: 'Colombo',
          urgency: ServiceRequestUrgency.low,
          status: ServiceRequestStatus.created,
          createdAt: DateTime(2026, 9, 9),
        ...
        mockService.analyzeException = DioException(
          requestOptions: RequestOptions(
            path: '/service-requests/req-rec-a/analyze',
          ),
    ...
    testWidgets(
      'timeout followed by grace-period expiry shows informational processing card, no red error, and Refresh Status button',
      (tester) async {
        final sample = ServiceRequestModel(
        ...
        mockService.analyzeException = DioException(
          requestOptions: RequestOptions(
            path: '/service-requests/req-grace/analyze',
          ),
          type: DioExceptionType.receiveTimeout,
        );
    ...
    testWidgets(
      'manual Refresh Status from grace-period expiry recovers to Analyzed status',
      (tester) async {
        final sample = ServiceRequestModel(
        ...
        await tester.pumpWidget(
          buildApp(
            const ServiceRequestDetailScreen(requestId: 'req-recover-a'),
          ),
        );
        await tester.pumpAndSettle();
        ...
        mockService.analyzeException = DioException(
          requestOptions: RequestOptions(
            path: '/service-requests/req-recover-a/analyze',
          ),
          type: DioExceptionType.receiveTimeout,
        );
  ...
  group('EditServiceRequestScreen', () {
    testWidgets('pre-fills existing request details and saves update', (
      tester,
    ) async {
      final sample = ServiceRequestModel(
  ...
  group('CreateServiceRequestScreen CategoryHint Integration', () {
    testWidgets(
      'submits canonical categoryHint for Plumbing and preserves description',
      (tester) async {
        await tester.pumpWidget(
          buildApp(
            const CreateServiceRequestScreen(
        ...
        expect(
          mockService.lastCreateDto?.description,
          'Bathroom pipe leaking under the washbasin',
        );
        expect(
    ...
    testWidgets(
      'submits Vehicle Assistance mapped to canonical Vehicle Repair',
      (tester) async {
        await tester.pumpWidget(
          buildApp(
            const CreateServiceRequestScreen(
    ...
    testWidgets('submits canonical categoryHint for Electrical', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildApp(
          const CreateServiceRequestScreen(
            initialCategoryPreference: 'Electrical',
          ),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets('submits canonical categoryHint for Appliance Repair', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildApp(
          const CreateServiceRequestScreen(
            initialCategoryPreference: 'Appliance Repair',
          ),
        ),
      );
      await tester.pumpAndSettle();
    ...
    testWidgets(
      'submits null categoryHint when Let AssistLK AI identify is chosen',
      (tester) async {
        await tester.pumpWidget(
          buildApp(
            const CreateServiceRequestScreen(initialCategoryPreference: null),
  ...
  group('EditServiceRequestScreen CategoryHint Integration', () {
    ...
    testWidgets(
      'preserves existing categoryHint when only description is modified',
      (tester) async {
        final sample = ServiceRequestModel(
        ...
        expect(
          mockService.lastUpdateDto?.description,
          'Updated description for plumbing problem',
        );
      },
    );
    ...
    testWidgets(
      'preserves existing categoryHint when only location is modified',
      (tester) async {
        final sample = ServiceRequestModel(
        ...
        expect(
          mockService.lastUpdateDto?.locationText,
          'New Address Colombo 03',
        );
      },
    );
    ...
    testWidgets(
      'updates categoryHint when customer chooses a different preference',
      (tester) async {
        final sample = ServiceRequestModel(
    ...
    testWidgets(
      'updates categoryHint to canonical Vehicle Repair when customer chooses Vehicle Assistance',
      (tester) async {
        final sample = ServiceRequestModel(
    ...
    testWidgets(
      'clears categoryHint with explicit null when customer chooses Let AssistLK AI identify',
      (tester) async {
        final sample = ServiceRequestModel(
  ...
  group(
    'ServiceRequestDetailScreen Preference and Lifecycle AI Classification',
    () {
      testWidgets(
        'shows Service preference and lifecycle wording in Created status',
        (tester) async {
          final sample = ServiceRequestModel(
          ...
          await tester.pumpWidget(
            buildApp(
              const ServiceRequestDetailScreen(requestId: 'req-detail-c'),
            ),
          );
          await tester.pumpAndSettle();
      ...
      testWidgets('shows Analysis in progress in Analyzing status', (
        tester,
      ) async {
        final sample = ServiceRequestModel(
        ...
        await tester.pumpWidget(
          buildApp(const ServiceRequestDetailScreen(requestId: 'req-detail-a')),
        );
        await tester.pump();
        await tester.pump();
      ...
      testWidgets(
        'shows Needs more information in AwaitingInformation status',
        (tester) async {
          final sample = ServiceRequestModel(
          ...
          await tester.pumpWidget(
            buildApp(
              const ServiceRequestDetailScreen(requestId: 'req-detail-ai'),
            ),
          );
          await tester.pumpAndSettle();
      ...
      testWidgets(
        'ReadyForMatching preserves AnalysisResultCard with completed AssistLK AI classification, handoff section, and no duplicate header rows',
        (tester) async {
          final sample = ServiceRequestModel(
          ...
          await tester.pumpWidget(
            buildApp(
              const ServiceRequestDetailScreen(requestId: 'req-detail-rfm'),
            ),
          );
          await tester.pumpAndSettle();
      ...
      testWidgets(
        'ReadyForMatching with matching hint/category shows no mismatch notice',
        (tester) async {
          final sample = ServiceRequestModel(
          ...
          await tester.pumpWidget(
            buildApp(
              const ServiceRequestDetailScreen(
                requestId: 'req-detail-rfm-match',
              ),
            ),
          );
          await tester.pumpAndSettle();
      ...
      testWidgets(
        'transition from Analyzed to ReadyForMatching does not cause analysis result to disappear',
        (tester) async {
          final sample = ServiceRequestModel(
          ...
          await tester.pumpWidget(
            buildApp(
              const ServiceRequestDetailScreen(requestId: 'req-transition'),
            ),
          );
          await tester.pumpAndSettle();
      ...
      testWidgets(
        'in Analyzed status, avoids duplicate rows in Header Card and delegates to AnalysisResultCard',
        (tester) async {
          final sample = ServiceRequestModel(
  ...
  group('AnalysisResultCard Mismatch Display Safety', () {
    testWidgets(
      'matching category and hint shows no mismatch banner in ReadyForMatching status',
      (tester) async {
    ...
    testWidgets(
      'mismatching category and hint in ReadyForMatching status shows neutral mismatch banner',
      (tester) async {
        final analysis = ProblemUnderstandingResultModel(
          workflowId: 'wf-rfm-2',
    ...
    testWidgets(
      'matching category and hint shows no mismatch banner in Analyzed status',
      (tester) async {
        final analysis = ProblemUnderstandingResultModel(
          workflowId: 'wf-1',
          executionId: 'ex-1',
        ...
        expect(
          find.text('Plumbing'),
          findsNWidgets(2),
        ); // in preference and AI classification
        expect(
          find.text(
            'AssistLK AI identified a different service category based on your problem description.',
          ),
    ...
    testWidgets(
      'mismatching category and hint in Analyzed status shows neutral mismatch banner',
      (tester) async {
        final analysis = ProblemUnderstandingResultModel(
          workflowId: 'wf-2',
          executionId: 'ex-2',
    ...
    testWidgets(
      'canonical equivalence between Vehicle Assistance and Vehicle Repair produces no mismatch',
      (tester) async {
        final analysis = ProblemUnderstandingResultModel(
          workflowId: 'wf-3',
          executionId: 'ex-3',
    ...
    testWidgets(
      'null categoryHint (Let AssistLK AI identify) shows no mismatch banner',
      (tester) async {
        final analysis = ProblemUnderstandingResultModel(
          workflowId: 'wf-4',
          executionId: 'ex-4',
    ...
    testWidgets('Unclassified AI category shows no mismatch banner', (
      tester,
    ) async {
      final analysis = ProblemUnderstandingResultModel(
        workflowId: 'wf-5',
    ...
    testWidgets(
      'lifecycle safety: mismatch notice suppressed when status is Created, Analyzing, AwaitingInformation, Cancelled',
      (tester) async {
        final analysis = ProblemUnderstandingResultModel(
          workflowId: 'wf-6',
          executionId: 'ex-6',
  ...
  group('Real GPS Service Location Feature 2', () {
    ...
    testWidgets(
      '1. Manual location flow works without GPS (coordinates remain null, zero GPS calls)',
      (tester) async {
        await tester.pumpWidget(
          buildApp(
        ...
        expect(
          find.byKey(const Key('use_current_location_button')),
          findsOneWidget,
        );
        await tester.enterText(
          find.widgetWithText(TextFormField, 'Location / Address'),
    ...
    testWidgets(
      '2. GPS success populates latitude and longitude into DTO and displays in review',
      (tester) async {
        await tester.pumpWidget(
          buildApp(
            CreateServiceRequestScreen(
        ...
        await tester.ensureVisible(
          find.byKey(const Key('edit_keep_gps_button')),
        );
    ...
    testWidgets('3. Remove GPS button clears captured coordinates', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildApp(
    ...
    testWidgets(
      '4. Permission denied shows explanation and allows manual fallback',
      (tester) async {
    ...
    testWidgets(
      '5. Permission permanently denied shows settings guidance and manual fallback',
      (tester) async {
        mockLocationService.permissionStatus =
    ...
    testWidgets(
      '6. Location services disabled shows turn-on guidance and manual fallback',
      (tester) async {
    ...
    testWidgets(
      '7. GPS timeout shows timeout message without blocking manual entry',
      (tester) async {
        mockLocationService.customResult = const LocationResult(
          status: LocationAccessStatus.timeout,
          message: 'Could not get your current location in time. Please enter the location manually.',
        );
    ...
    testWidgets('8. GPS exception handled safely without crashing UI', (
      tester,
    ) async {
      ...
      expect(
        find.text(
          'Could not resolve your location. Please enter the location manually or retry.',
        ),
        findsOneWidget,
      );
    });

    testWidgets('9. CategoryHint and GPS coordinates submitted together', (
      tester,
    ) async {
      await tester.pumpWidget(
        buildApp(
    ...
    testWidgets(
      '10. EditServiceRequestScreen preserves coordinates when locationText is unchanged',
      (tester) async {
    ...
    testWidgets(
      '11. EditServiceRequestScreen blocks PUT and shows confirmation when locationText changes with GPS',
      (tester) async {
        final existingReq = ServiceRequestModel(
        ...
        expect(
          find.byKey(const Key('edit_gps_confirmation_prompt')),
          findsOneWidget,
        );
        ...
        expect(
          find.text(
            'Please confirm the location and attached GPS coordinates.',
          ),
          findsOneWidget,
        );
      },
    ...
    testWidgets(
      '12. EditServiceRequestScreen allows keeping existing coordinates on confirmation',
      (tester) async {
        final existingReq = ServiceRequestModel(
    ...
    testWidgets(
      '13. EditServiceRequestScreen allows removing GPS coordinates on confirmation',
      (tester) async {
        final existingReq = ServiceRequestModel(
    ...
    testWidgets(
      '14. EditServiceRequestScreen allows updating with current location on confirmation',
      (tester) async {
        final existingReq = ServiceRequestModel(
        ...
        expect(
          mockService.lastUpdateDto!.toJson()['locationSource'],
          'OpenStreetMap',
        );
    ...
    testWidgets(
      '15. ServiceRequestDetailScreen displays GPS location captured when coordinates are present',
      (tester) async {
        ...
        await tester.pumpWidget(
          buildApp(
            const ServiceRequestDetailScreen(requestId: 'req-detail-gps'),
          ),
        );
        await tester.pumpAndSettle();
